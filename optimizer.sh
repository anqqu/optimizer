...
# Критически важно при запуске через "curl | bash":
# stdin занят пайпом (читает сам скрипт), поэтому read получает пустую строку
# мгновенно и меню крутится бесконечно. exec < /dev/tty переключает stdin
# обратно на терминал — теперь read нормально ждёт ввода пользователя.
exec < /dev/tty
